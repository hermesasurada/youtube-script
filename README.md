...
| `MAX_VISUAL_TARGETS` / `TARGET_WINDOW` / `TARGET_INTERVAL` | 12 / 3 / 2 | 전사의 시각 자료 언급 목표 수 / 목표 전후 탐색 범위(초) / 주변 프레임 간격(초) |
| `MAX_FRAMES_PER_SECTION` | 3 | 점수 판정된 최종 캡처의 소제목별 상한 |
| `PROBE_TIMEOUT` / `FFMPEG_TIMEOUT` / `VISION_TIMEOUT` | 30 / 600 / 300 | subprocess 타임아웃(초) |
| `MONITOR_MAX_PIPELINE_ATTEMPTS` | 5 | 자동 처리 최대 시도 횟수 |
| `MONITOR_RETRY_BASE_SEC` | 1200 | 파이프라인 실패 재시도 간격(20분 고정) |

## 키프레임 추출 개요

1. yt-dlp로 저해상도 영상 다운로드(임시 디렉터리).
2. **전사 기반 후보 보강** — 타임스탬프 전사에서 차트·사진·화면·데모 등의 언급을 찾아 전후 프레임을 추가한다.
3. **하이브리드 후보 추출** — 전사 목표 + 균등 간격(`fps=1/N`, 전 구간 커버) + 장면전환(`select='gt(scene,…)'`).
4. **근접 중복 제거**(6초) — 목표 주변과 장면전환 프레임은 비교용으로 보존하고 인접 균등 프레임만 솎는다. 모델 입력 상한은 40장이다.
5. **문맥형 비전 분류** — 이미지와 앞뒤 15초 전사·소제목을 함께 보고 관련성·정보량·가독성을 평가한다. 토킹헤드·청중·장식 B-roll·오프닝·흐림·중복은 계속 제외한다. 자동모니터 팝업에 저장한 Opus 5/GPT-6 Astra/Grok-4.5 순서대로 폴백하며, 수동 캡처는 기존 Opus 5 → Grok-4.5 정책을 유지한다.
6. 비전 모델의 의미 기반 소제목 배치를 우선하고, 미판정·잘못된 번호에만 시간 라벨(`[mm:ss]`)을 폴백으로 적용해 요약 소제목 직후에 캡처를 주입한다.
